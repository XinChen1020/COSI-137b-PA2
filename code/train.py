...
parser.add_argument('--dense_dan_layers', type=str, default='100,100',
                    help='List if integer dense unit layers (DAN only)')
...
parser.add_argument('--embedding_size', type=int, default=100, help='Embedding size (if random) (DEFAULT = 100)')
...
parser.add_argument('--use_llm_embeddings', action='store_true',
                    help='Use an LLM as a feature extractor (no fine-tuning)', default=False)
...
parser.add_argument('--llm_proj_dim', type=int,
                    help='Down-projected dimensions (0 indicates no projection is performed)', default=0)
...
if __name__ == '__main__':
    ...
    if args.use_llm_embeddings:
        ...
        model = get_model(len(label_map), vocab_size=vocab_size, embedding_size=embedding_size,

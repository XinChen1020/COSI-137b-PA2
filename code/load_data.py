...
from transformers import DistilBertTokenizer, DistilBertModel, OpenAIGPTTokenizer
...
def get_hf_loader(data, embedding_model, text_pipeline, label_pipeline, batch_size, max_len,
    ...
    # Since GPT model does not have pretrain for padding,
    # to create cache embedding file, uncomment the 2 lines below. Comment when finishing creating cache for openai-gpt
    # text_pipeline.add_special_tokens({'pad_token': '[PAD]'})
    # embedding_model.resize_token_embeddings(len(text_pipeline))
...
def get_hf_data_loaders(train_file, val_file, test_file, batch_size, max_len, label_map,
    ...
    # temp_data_loaders(train_file, val_file, test_file, batch_size, max_len, label_map,
    #                     hf_llm_name,
    #                     cache_dir,
    #                     proj_size)
